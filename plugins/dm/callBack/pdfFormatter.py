...
async def _formatter(bot, callbackQuery):
    try:
        ...
        if downloadLoc is None:
            ...
            return
        await downloadMessage.edit("`بدأ التنسيق Started Formatting ..` 📖")
            ...
            with fitz.open() as outPDF:
                ...
                if nOfPages > 5:
                    await downloadMessage.edit(
                        text="__بالنسبة لبعض الأسباب ، يدعم تنسيق A4 ملفات PDF التي تحتوي على أقل من 5 صفحات__\n __For Some Reason A4 FORMATTING Supports for pdfs with less than 5 Pages__"
                             f"\n\nعدد الصفحات (Total Pages): {nOfPages} ⭐"
                    )
                    PROCESS.remove(callbackQuery.message.chat.id)
        ...
        await callbackQuery.message.reply_document(
            ...
            thumb=PDF_THUMBNAIL, caption="ملف PDF منسق (A4) | formated PDF (A4)"
        )
        await downloadMessage.delete()
        PROCESS.remove(callbackQuery.message.chat.id)
        shutil.rmtree(f"{callbackQuery.message.message_id}")
